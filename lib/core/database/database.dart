...
class Exercises extends Table {
  ...
  TextColumn get muscleGroup =>
      text().nullable()(); // Comma-separated: "Chest,Shoulders,Triceps"
  BoolColumn get isCardio => boolean().withDefault(const Constant(false))();
  TextColumn get cardioType =>
      text().nullable()(); // LISS, HIIT, or null for non-cardio
}
...
class FoodLogs extends Table {
  ...
  BoolColumn get isEaten =>
      boolean().withDefault(const Constant(true))(); // For planning mode
}
...
class CaloriePlans extends Table {
  IntColumn get id => integer().autoIncrement()();
  TextColumn get name => text().withLength(min: 1, max: 120)();
  TextColumn get goalType =>
      text().withDefault(const Constant('maintain'))(); // maintain, gain, cut
  DateTimeColumn get startDate => dateTime()();
  DateTimeColumn get endDate => dateTime()();
  IntColumn get targetDailyCalories =>
      integer().nullable()(); // Optional daily intake target
  IntColumn get targetWeeklyDeficit =>
      integer().nullable()(); // +deficit for cut, -deficit for gain
  BoolColumn get isActive => boolean().withDefault(const Constant(true))();
  DateTimeColumn get createdAt => dateTime().withDefault(currentDateAndTime)();
}

class CaloriePlanWeeks extends Table {
  IntColumn get id => integer().autoIncrement()();
  IntColumn get planId => integer().references(CaloriePlans, #id)();
  IntColumn get weekNumber => integer()(); // 1-based week index
  DateTimeColumn get weekStartDate => dateTime()();
  DateTimeColumn get weekEndDate => dateTime()();
  IntColumn get targetDailyCalories => integer().nullable()();
  IntColumn get targetWeeklyDeficit => integer().nullable()();
...
class DailyEnergyLogs extends Table {
  IntColumn get id => integer().autoIncrement()();
  DateTimeColumn get logDate => dateTime()(); // Start-of-day normalized date
  IntColumn get steps => integer().withDefault(const Constant(0))();
  RealColumn get caloriesOut =>
      real()(); // User-provided daily calories out / TDEE
  TextColumn get source =>
      text().withDefault(const Constant('manual'))(); // manual only for now
  TextColumn get notes => text().nullable()();
...
class BmrProfiles extends Table {
  IntColumn get id => integer().autoIncrement()();
...
class BodyFatLogs extends Table {
  ...
  TextColumn get method => text().withDefault(
    const Constant('estimate'),
  )(); // scale, caliper, dexa, estimate
  TextColumn get notes => text().nullable()();
}
...
class Expenses extends Table {
  ...
  IntColumn get linkedFoodLogId => integer().nullable().references(
    FoodLogs,
    #id,
  )(); // for cross-domain linking
}
...
class AppDatabase extends _$AppDatabase {
  ...
  @override
  int get schemaVersion => 7; // v7: Added BMR profile storage
  ...
  MigrationStrategy get migration {
    return MigrationStrategy(
      ...
      onUpgrade: (Migrator m, int from, int to) async {
        ...
        // Migration from version 5 to 6: Add calorie planning + daily energy tracking tables
        if (from < 6) {
          await m.createTable(caloriePlans);
          await m.createTable(caloriePlanWeeks);
          await m.createTable(dailyEnergyLogs);
        }
        // Migration from version 6 to 7: Add BMR profile table
        if (from < 7) {
          await m.createTable(bmrProfiles);
        }
      },
  ...
  Future<void> _seedExercises() async {
    final exercises = [
      ...
      ExercisesCompanion.insert(
        name: 'Bench Press',
        category: 'Push',
        muscleGroup: const Value('Chest,Triceps,Shoulders'),
      ),
      ExercisesCompanion.insert(
        name: 'Incline Bench Press',
        category: 'Push',
        muscleGroup: const Value('Chest,Shoulders,Triceps'),
      ),
      ExercisesCompanion.insert(
        name: 'Overhead Press',
        category: 'Push',
        muscleGroup: const Value('Shoulders,Triceps'),
      ),
      ExercisesCompanion.insert(
      ...
      ExercisesCompanion.insert(
        ...
        category: 'Push',
        muscleGroup: const Value('Triceps'),
      ),
      ExercisesCompanion.insert(
        name: 'Dips',
        category: 'Push',
      ...
      ExercisesCompanion.insert(
        name: 'Deadlift',
        category: 'Pull',
        muscleGroup: const Value('Back,Hamstrings,Glutes'),
      ),
      ExercisesCompanion.insert(
        name: 'Pull-ups',
        category: 'Pull',
        muscleGroup: const Value('Back,Biceps'),
      ),
      ExercisesCompanion.insert(
        name: 'Barbell Row',
        category: 'Pull',
      ...
      ExercisesCompanion.insert(
        name: 'Bicep Curls',
        category: 'Pull',
        muscleGroup: const Value('Biceps,Forearms'),
      ),
      ExercisesCompanion.insert(
        name: 'Face Pulls',
        category: 'Pull',
        muscleGroup: const Value('Shoulders,Back'),
      ),
      // Legs
      ExercisesCompanion.insert(
        name: 'Squat',
        category: 'Legs',
        muscleGroup: const Value('Quads,Glutes,Hamstrings'),
      ),
      ExercisesCompanion.insert(
        name: 'Leg Press',
        category: 'Legs',
        muscleGroup: const Value('Quads,Glutes'),
      ),
      ExercisesCompanion.insert(
        name: 'Romanian Deadlift',
        category: 'Legs',
        muscleGroup: const Value('Hamstrings,Glutes,Back'),
      ),
      ExercisesCompanion.insert(
        name: 'Leg Curl',
        category: 'Legs',
        muscleGroup: const Value('Hamstrings'),
      ),
      ExercisesCompanion.insert(
        name: 'Leg Extension',
        category: 'Legs',
      ...
      ExercisesCompanion.insert(
        name: 'Calf Raises',
        category: 'Legs',
        muscleGroup: const Value('Calves'),
      ),
      // Cardio - LISS
      ExercisesCompanion.insert(
        name: 'Running',
        category: 'Cardio',
        isCardio: const Value(true),
        cardioType: const Value('LISS'),
      ),
      ExercisesCompanion.insert(
        name: 'Cycling',
        category: 'Cardio',
  ...
  Future<void> _seedExpenseCategories() async {
    final categories = [
      ExpenseCategoriesCompanion.insert(
        name: 'Food & Dining',
        icon: '🍽️',
        isFoodRelated: const Value(true),
      ),
      ExpenseCategoriesCompanion.insert(
        name: 'Groceries',
        icon: '🛒',
  ...
  Future<void> _seedSupplements() async {
    final supps = [
      SupplementsCompanion.insert(
        name: 'Whey Protein',
        type: 'protein',
        dosageUnit: const Value('scoop'),
      ),
      SupplementsCompanion.insert(
        name: 'Creatine',
        type: 'performance',
        dosageUnit: const Value('g'),
      ),
      SupplementsCompanion.insert(
        name: 'Vitamin D3',
        type: 'vitamin',
      ...
      SupplementsCompanion.insert(
        name: 'Vitamin B12',
        type: 'vitamin',
        dosageUnit: const Value('mcg'),
      ),
      SupplementsCompanion.insert(
        name: 'Omega-3 Fish Oil',
        type: 'fatty_acid',
        dosageUnit: const Value('capsule'),
      ),
      SupplementsCompanion.insert(
        name: 'Zinc',
        type: 'mineral',
